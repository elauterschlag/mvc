...
class Validate {
	...
	// display errors especially for bootstrap
	public function displayErrors() {
		//$html = '<h3 class="text-white m-4">Error</h3>';
		$html = '<ul>';
		foreach($this->_errors as $error) {
			if(is_array($error)) {
				$html .= '<li class="text-white font-weight-bold">' . $error[0] . '</li>';
				$html .= '<script>jQuery("document").ready(function(){jQuery("#' . $error[1] . '").addClass("is-invalid");});</script>';
			}else{
				$html .= '<li class="text-white font-weight-bold">' . $error . '</li>';
			}
		}
		$html .= '</ul>';
		return $html;
